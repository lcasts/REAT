...
def main():
    ...
    for idx, scenario_name in enumerate(df_scenarios.index, start=1):
        ...
        try:
            ...
            if df_trajaerodata is None or df_trajaerodata.empty:
                print(f"[{scenario_name}] No trajectory data available, skipping scenario.")
                continue  # skip to the next scenario
            ...
            print(f"Error calculating trajectory for scenario {scenario_name}: {e}")
        ...
        try: 
            if calculate_emissions:  
                ...
                if use_emission_factors and (traj_inp_data == "PyDrama" or traj_inp_data == "OwnDrama" or traj_inp_data == "OwnDebrisk"):
                    ...
                    data_processing.save_emission_results(filtered_emission_results, scenario_name,'Emission_Factor')
                    emissions.clean_up()
                if use_emission_factors and (traj_inp_data == "OwnScarab"):   
                ...
                if use_nasa_cea and (traj_inp_data == "PyDrama" or traj_inp_data == "OwnDrama" or traj_inp_data == "OwnDebrisk"):   
                    ...
                    filtered_emission_results = data_processing.filter_emission_data(emission_results, compress_method, compress_interval, compress_lat, compress_lon)
                    data_processing.save_emission_results(filtered_emission_results, scenario_name,'CEA')
                    emissions.clean_up()
                if use_nasa_cea and (traj_inp_data == "OwnScarab"):   
                    #print(df_trajectory_results_cleaned, df_phases, df_launch_vehicles_engines, df_launch_vehicles, scenario_name, df_emission_factors)
                    emission_results = emissions.run_nasa_cea_scarab(df_trajaerodata, df_emission_factors, df_material_data, scenario_name)
                    filtered_emission_results = data_processing.filter_emission_data(emission_results, compress_method, compress_interval, compress_lat, compress_lon)
                    data_processing.save_emission_results(filtered_emission_results, scenario_name,'CEA')
                    emissions.clean_up()
                if traj_inp_data == "Ballistic" or traj_inp_data == "OwnBallistic":
                    if nox_method == "nasa_cea": 
                        ...
                        emission_results = emissions.run_cantera_bal(df_trajaerodata, df_scenarios, scenario_name)
                        print('emission_results\n,',emission_results)
                        filtered_emission_results = data_processing.filter_emission_data_bal(emission_results, compress_method, compress_interval, compress_lat, compress_lon)
                        data_processing.save_emission_results(filtered_emission_results, scenario_name,'Cantera')
                        emissions.clean_up()
                if not (
                    (use_emission_factors and traj_inp_data in ["PyDrama", "OwnDrama", "OwnDebrisk"]) or
                    (use_emission_factors and traj_inp_data == "OwnScarab") or
                    (use_nasa_cea and traj_inp_data in ["PyDrama", "OwnDrama", "OwnDebrisk"]) or
                    (use_nasa_cea and traj_inp_data == "OwnScarab") or
                    (traj_inp_data in ["Ballistic", "OwnBallistic"])
                ):
                    print('Invalid combination. Please check your config settings')

        except Exception as e:
            print(f"Error calculating emission for scenario {scenario_name}: {e}")     
